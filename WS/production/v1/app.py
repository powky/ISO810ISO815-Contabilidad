...
def leer_asientocontable_bd_by_aux(id):
    try:
        sql = "SELECT * FROM accounting_entries WHERE aux_id = '{0}' ORDER BY id DESC".format(
            id
        )
        cursor.execute(sql)
        datos = cursor.fetchone()
...
def registrar_asientocontable():
    try:
        ...
        if error.code == 20001:
            ...
            return jsonify(
                {
                    "mensaje": "Error desconocido al registrar la transacción.",
                    "exito": False,
                }
            ), 400
    # Excepcion si el error es que no envio todos los parametros
        ...
        return jsonify(
            {"mensaje": "Error al registrar la transacción.", "exito": False}
        ), 400

def pagina_no_encontrada(error):
    return "<h4>Por favor utilice un endpoint válido. API Público de Contabilidad por Ian Fernández (A00100600) y Raymer Segura (A00104796).</h4>", 404
