...
class Ui_TabWidget(QtCore.QObject):
    ...
    def filter_accounting_accs(self):
        ...
        if any([accounting_filter_id, accounting_filter_description, accounting_filter_acc_type, accounting_filter_trx_allowed, accounting_filter_acc_lvl, accounting_filter_gnl_acc, accounting_filter_balance, accounting_filter_status]):
        ...
        status_mapping = {
            "activado": "1", "Acivado": "1", "Desactivado": "0", "desactivado": "0", "1": "1", "0": "0", "off": "0", "on": "1", "Off": "0", "On": "1"
        }
    ...
    def update_accounting_data(self):
        ...
        try:
            ...
            if acc_type != acc_type_mapping.get(updated_acc_type, acc_type):
                update_query += "acc_types = :acc_type, "
                update_params["acc_type"] = acc_type_mapping.get(updated_acc_type, acc_type)
            if trx_allowed != trx_allowed_mapping.get(updated_trx_allowed, trx_allowed):
            ...
            if status != status_mapping.get(updated_status, status):
    ...
    # Funciones encargadas de actualizar el UI masivamente
    ...
    def update_button_enabled(self):
        ...
        self.accountingInsertBtn.setEnabled(enable_button_accounting)

        if self.select_fields is None:
            # No selected fields stored, disable the update button
            self.accountingUpdateBtn.setEnabled(False)
        else:
            # Compare the updated values with the stored selected fields
            if (
                accounting_id == self.select_fields['accounting_id'] and
                accounting_description == self.select_fields['accounting_description'] and
                accounting_acc_type == self.select_fields['accounting_acc_type'] and
                accounting_trx_allowed == self.select_fields['accounting_trx_allowed'] and
                accounting_acc_lvl == self.select_fields['accounting_acc_lvl'] and
                accounting_gnl_acc == self.select_fields['accounting_gnl_acc'] and
                accounting_balance == self.select_fields['accounting_balance'] and
                accounting_status == self.select_fields['accounting_status']
            ):
                self.accountingUpdateBtn.setEnabled(False)
            else:
                self.accountingUpdateBtn.setEnabled(True)

        enable_button_acctype = all([acctype_id, acctype_desc, acctype_orig, acctype_sts])
        self.acctypeInsertBtn.setEnabled(enable_button_acctype)
        ...
        self.currencyInsertBtn.setEnabled(enable_button_currency)

        enable_button_aux = all([aux_id, aux_desc, aux_sts])
        self.auxInsertBtn.setEnabled(enable_button_aux)
